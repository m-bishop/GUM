...
public class ActionItemHold extends Action {
	
	public ActionItemHold(){
	}
	
	public void init(){
		this.setActionName("ItemHold");
	...
	public boolean doAction(ActionHeader header) {
		...
		System.out.println(player.getCommandST().nextToken());
			if (player.getCommandST().hasMoreTokens()) {
				if (player.getCommand().contains("left")) {
					player.holdLeft(item);
				} else {
					player.holdRight(item);
				}
			} else {
				player.holdRight(item);
			}
	...
	public void menu(User u) throws MenuExitException {
		...
		String menuString = "Configure Hold Item Action:\r\n";
